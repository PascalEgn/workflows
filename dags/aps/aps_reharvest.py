...
@dag(
    ...
    params={
        ...
        "file_keys": Param(
            default=[],
            type=["array", "null"],
            description="List of file keys to process",
            title="File keys",
        ),
        "dois": Param(
            default=[],
...
def aps_reharvest():
    ...
    def collect_articles(repo=APS_REPO, **kwargs):
        ...
        if file_keys:
            logger.info("Selecting APS snapshot files by explicit keys")
            missing_keys = [
                key for key in file_keys if key not in all_snapshot_keys_set
            ]
            if missing_keys:
                raise ValueError(
                    f"Some requested file_keys do not exist: {missing_keys}"
                )
            selected_keys = list(file_keys)
